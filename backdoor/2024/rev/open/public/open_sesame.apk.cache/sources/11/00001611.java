...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m129constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m129constructorimpl = Result.m129constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
        ...
        if (Result.m132exceptionOrNullimpl(m129constructorimpl) != null) {
            m129constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
