...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
        ...
        loop0: while (true) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    m1686constructorimpl = SegmentOrClosed.m1686constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1691isClosedimpl(m1686constructorimpl)) {
                Segment m1689getSegmentimpl = SegmentOrClosed.m1689getSegmentimpl(m1686constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m1689getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m1689getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1689getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m1689getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1689getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1689getSegmentimpl(m1686constructorimpl);
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m1689getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m1689getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1689getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
