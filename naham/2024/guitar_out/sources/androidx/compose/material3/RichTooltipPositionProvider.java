...
final /* data */ class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long mo874calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        ...
        if (IntSize.m5033getWidthimpl(popupContentSize) + right > IntSize.m5033getWidthimpl(windowSize) && (right = anchorBounds.getLeft() - IntSize.m5033getWidthimpl(popupContentSize)) < 0) {
            right = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m5033getWidthimpl(popupContentSize)) / 2);
        }
        int top = (anchorBounds.getTop() - IntSize.m5032getHeightimpl(popupContentSize)) - this.tooltipAnchorPadding;
        if (top < 0) {
