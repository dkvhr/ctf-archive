...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> color, State<RippleAlpha> rippleAlpha, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.INSTANCE.m2468getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo3172getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m960getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3172getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo319roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m2639unboximpl = this.color.getValue().m2639unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m965drawStateLayerH2RKhps(contentDrawScope2, this.radius, m2639unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m964updateRipplePropertiesbiQXAtU(contentDrawScope.mo3172getSizeNHjbRc(), this.rippleRadius, m2639unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m963addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m2639unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
