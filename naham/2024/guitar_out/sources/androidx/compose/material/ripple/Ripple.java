...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo956rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer composer, int i) {
        ...
        if (this.color.getValue().m2639unboximpl() != Color.INSTANCE.m2665getUnspecified0d7_KjU()) {
            mo958defaultColorWaAFU9c = this.color.getValue().m2639unboximpl();
        } else {
            mo958defaultColorWaAFU9c = rippleTheme.mo958defaultColorWaAFU9c(composer, 0);
        }
        composer.endReplaceableGroup();
        RippleIndicationInstance mo956rememberUpdatedRippleInstance942rkJo = mo956rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, SnapshotStateKt.rememberUpdatedState(Color.m2619boximpl(mo958defaultColorWaAFU9c), composer, 0), SnapshotStateKt.rememberUpdatedState(rippleTheme.rippleAlpha(composer, 0), composer, 0), composer, (i & 14) | ((i << 12) & 458752));
        EffectsKt.LaunchedEffect(mo956rememberUpdatedRippleInstance942rkJo, interactionSource, new Ripple$rememberUpdatedInstance$1(interactionSource, mo956rememberUpdatedRippleInstance942rkJo, null), composer, ((i << 3) & 112) | 520);
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        composer.endReplaceableGroup();
        return mo956rememberUpdatedRippleInstance942rkJo;
    }
    ...
    public boolean equals(Object other) {
        ...
        return this.bounded == ripple.bounded && Dp.m4878equalsimpl0(this.radius, ripple.radius) && Intrinsics.areEqual(this.color, ripple.color);
    }

    public int hashCode() {
        return (((Boolean.hashCode(this.bounded) * 31) + Dp.m4879hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
