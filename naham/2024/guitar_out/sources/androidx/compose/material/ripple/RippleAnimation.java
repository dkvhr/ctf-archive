...
public final class RippleAnimation {
    ...
    */
    ...
    public final void m959draw4WTKRHQ(DrawScope draw, long j) {
        Float valueOf;
        Intrinsics.checkNotNullParameter(draw, "$this$draw");
        if (this.startRadius == null) {
            this.startRadius = Float.valueOf(RippleAnimationKt.m961getRippleStartRadiusuvyYCjk(draw.mo3172getSizeNHjbRc()));
        }
        if (this.targetRadius == null) {
            if (Float.isNaN(this.radius)) {
                valueOf = Float.valueOf(RippleAnimationKt.m960getRippleEndRadiuscSwnlzA(draw, this.bounded, draw.mo3172getSizeNHjbRc()));
            } else {
                valueOf = Float.valueOf(draw.mo325toPx0680j_4(this.radius));
            }
            this.targetRadius = valueOf;
        }
        if (this.origin == null) {
            this.origin = Offset.m2379boximpl(draw.mo3171getCenterF1C5BW0());
        }
        if (this.targetCenter == null) {
            this.targetCenter = Offset.m2379boximpl(OffsetKt.Offset(Size.m2459getWidthimpl(draw.mo3172getSizeNHjbRc()) / 2.0f, Size.m2456getHeightimpl(draw.mo3172getSizeNHjbRc()) / 2.0f));
        }
        ...
        float m2390getXimpl = Offset.m2390getXimpl(offset.getPackedValue());
        Offset offset2 = this.targetCenter;
        Intrinsics.checkNotNull(offset2);
        float lerp2 = MathHelpersKt.lerp(m2390getXimpl, Offset.m2390getXimpl(offset2.getPackedValue()), this.animatedCenterPercent.getValue().floatValue());
        Offset offset3 = this.origin;
        Intrinsics.checkNotNull(offset3);
        float m2391getYimpl = Offset.m2391getYimpl(offset3.getPackedValue());
        Offset offset4 = this.targetCenter;
        Intrinsics.checkNotNull(offset4);
        long Offset = OffsetKt.Offset(lerp2, MathHelpersKt.lerp(m2391getYimpl, Offset.m2391getYimpl(offset4.getPackedValue()), this.animatedCenterPercent.getValue().floatValue()));
        long m2628copywmQWz5c$default = Color.m2628copywmQWz5c$default(j, Color.m2631getAlphaimpl(j) * floatValue, 0.0f, 0.0f, 0.0f, 14, null);
        if (!this.bounded) {
            DrawScope.m3154drawCircleVaOC9Bg$default(draw, m2628copywmQWz5c$default, lerp, Offset, 0.0f, null, null, 0, MenuKt.InTransitionDuration, null);
            return;
        }
        float m2459getWidthimpl = Size.m2459getWidthimpl(draw.mo3172getSizeNHjbRc());
        float m2456getHeightimpl = Size.m2456getHeightimpl(draw.mo3172getSizeNHjbRc());
        int m2618getIntersectrtfAjoo = ClipOp.INSTANCE.m2618getIntersectrtfAjoo();
        DrawContext drawContext = draw.getDrawContext();
        long mo3097getSizeNHjbRc = drawContext.mo3097getSizeNHjbRc();
        drawContext.getCanvas().save();
        drawContext.getTransform().mo3100clipRectN_I0leg(0.0f, 0.0f, m2459getWidthimpl, m2456getHeightimpl, m2618getIntersectrtfAjoo);
        DrawScope.m3154drawCircleVaOC9Bg$default(draw, m2628copywmQWz5c$default, lerp, Offset, 0.0f, null, null, 0, MenuKt.InTransitionDuration, null);
