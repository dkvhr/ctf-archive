...
public final class RippleHostView extends View {
    ...
    public final void m963addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m964updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            unprojectedRipple.setHotspot(Offset.m2390getXimpl(interaction.getPressPosition()), Offset.m2391getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m964updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple unprojectedRipple = this.ripple;
        if (unprojectedRipple == null) {
            return;
        }
        unprojectedRipple.trySetRadius(radius);
        unprojectedRipple.m971setColorDxMtmZc(color, alpha);
        Rect rect = new Rect(0, 0, MathKt.roundToInt(Size.m2459getWidthimpl(size)), MathKt.roundToInt(Size.m2456getHeightimpl(size)));
        setLeft(rect.left);
        setTop(rect.top);
        setRight(rect.right);
