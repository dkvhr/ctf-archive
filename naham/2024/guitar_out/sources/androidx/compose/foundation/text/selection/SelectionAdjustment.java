...
public interface SelectionAdjustment {
    ...
    long mo885adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange);
    ...
    public static final class Companion {
        ...
        private static final SelectionAdjustment None = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$None$1
            ...
            public long mo885adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
        ...
        private static final SelectionAdjustment Character = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Character$1
            ...
            public long mo885adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                if (TextRange.m4398getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4404getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, previousSelectionRange != null ? TextRange.m4403getReversedimpl(previousSelectionRange.getPackedValue()) : false);
                }
                return newRawSelectionRange;
            }
        };
        private static final SelectionAdjustment Word = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Word$1
            ...
            /* renamed from: adjust-ZXO7KMw */
            public long mo885adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                long m887adjustByBoundaryDvylE;
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                m887adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m887adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Word$1$adjust$1(textLayoutResult));
                return m887adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment Paragraph = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Paragraph$1
            ...
            public long mo885adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                long m887adjustByBoundaryDvylE;
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                m887adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m887adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Paragraph$1$adjust$boundaryFun$1(textLayoutResult.getLayoutInput().getText()));
                return m887adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment CharacterWithWordAccelerate = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$CharacterWithWordAccelerate$1
            ...
            public long mo885adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                if (previousSelectionRange == null) {
                    return SelectionAdjustment.Companion.$$INSTANCE.getWord().mo885adjustZXO7KMw(textLayoutResult, newRawSelectionRange, previousHandleOffset, isStartHandle, previousSelectionRange);
                }
                if (TextRange.m4398getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4404getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, TextRange.m4403getReversedimpl(previousSelectionRange.getPackedValue()));
                }
                if (isStartHandle) {
                    i = updateSelectionBoundary(textLayoutResult, TextRange.m4404getStartimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4404getStartimpl(previousSelectionRange.getPackedValue()), TextRange.m4399getEndimpl(newRawSelectionRange), true, TextRange.m4403getReversedimpl(newRawSelectionRange));
                    updateSelectionBoundary = TextRange.m4399getEndimpl(newRawSelectionRange);
                } else {
                    int m4404getStartimpl = TextRange.m4404getStartimpl(newRawSelectionRange);
                    updateSelectionBoundary = updateSelectionBoundary(textLayoutResult, TextRange.m4399getEndimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4399getEndimpl(previousSelectionRange.getPackedValue()), TextRange.m4404getStartimpl(newRawSelectionRange), false, TextRange.m4403getReversedimpl(newRawSelectionRange));
                    i = m4404getStartimpl;
                }
            ...
            private final int snapToWordBoundary(TextLayoutResult textLayoutResult, int newRawOffset, int currentLine, int otherBoundaryOffset, boolean isStart, boolean isReversed) {
                int lineStart;
                int lineEnd$default;
                long m4378getWordBoundaryjx7JFs = textLayoutResult.m4378getWordBoundaryjx7JFs(newRawOffset);
                if (textLayoutResult.getLineForOffset(TextRange.m4404getStartimpl(m4378getWordBoundaryjx7JFs)) == currentLine) {
                    lineStart = TextRange.m4404getStartimpl(m4378getWordBoundaryjx7JFs);
                } else {
                    lineStart = textLayoutResult.getLineStart(currentLine);
                }
                if (textLayoutResult.getLineForOffset(TextRange.m4399getEndimpl(m4378getWordBoundaryjx7JFs)) == currentLine) {
                    lineEnd$default = TextRange.m4399getEndimpl(m4378getWordBoundaryjx7JFs);
                } else {
            ...
            private final boolean isAtWordBoundary(TextLayoutResult textLayoutResult, int i) {
                long m4378getWordBoundaryjx7JFs = textLayoutResult.m4378getWordBoundaryjx7JFs(i);
                return i == TextRange.m4404getStartimpl(m4378getWordBoundaryjx7JFs) || i == TextRange.m4399getEndimpl(m4378getWordBoundaryjx7JFs);
            }
        };
        ...
        public final long m887adjustByBoundaryDvylE(TextLayoutResult textLayoutResult, long newRawSelection, Function1<? super Integer, TextRange> boundaryFun) {
            if (textLayoutResult.getLayoutInput().getText().length() == 0) {
                return TextRange.INSTANCE.m4409getZerod9O1mEE();
            }
            int lastIndex = StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText());
            long packedValue = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4404getStartimpl(newRawSelection), 0, lastIndex))).getPackedValue();
            long packedValue2 = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4399getEndimpl(newRawSelection), 0, lastIndex))).getPackedValue();
            return TextRangeKt.TextRange(TextRange.m4403getReversedimpl(newRawSelection) ? TextRange.m4399getEndimpl(packedValue) : TextRange.m4404getStartimpl(packedValue), TextRange.m4403getReversedimpl(newRawSelection) ? TextRange.m4404getStartimpl(packedValue2) : TextRange.m4399getEndimpl(packedValue2));
        }
    }
}
