...
public final class TextFieldSelectionManagerKt {
    ...
    public static final boolean isSelectionHandleInVisibleBound(TextFieldSelectionManager textFieldSelectionManager, boolean z) {
        ...
        return SelectionManagerKt.m927containsInclusiveUv8p0NA(visibleBounds, textFieldSelectionManager.m947getHandlePositiontuRUvjQ$foundation_release(z));
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m948calculateSelectionMagnifierCenterAndroidO0kMr_c(TextFieldSelectionManager manager, long j) {
        int m4404getStartimpl;
        ...
        if (i == 1 || i == 2) {
            m4404getStartimpl = TextRange.m4404getStartimpl(manager.getValue$foundation_release().getSelection());
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            m4404getStartimpl = TextRange.m4399getEndimpl(manager.getValue$foundation_release().getSelection());
        }
        int originalToTransformed = manager.getOffsetMapping().originalToTransformed(m4404getStartimpl);
        ...
        long m2420getCenterF1C5BW0 = value.getBoundingBox(coerceIn).m2420getCenterF1C5BW0();
        ...
        Offset m945getCurrentDragPosition_m7T9E = manager.m945getCurrentDragPosition_m7T9E();
        if (m945getCurrentDragPosition_m7T9E == null) {
            return Offset.INSTANCE.m2405getUnspecifiedF1C5BW0();
        }
        float m2390getXimpl = Offset.m2390getXimpl(innerTextFieldCoordinates.mo3874localPositionOfR5De75A(layoutCoordinates, m945getCurrentDragPosition_m7T9E.getPackedValue()));
        ...
        boolean z = TextRange.m4404getStartimpl(manager.getValue$foundation_release().getSelection()) > TextRange.m4399getEndimpl(manager.getValue$foundation_release().getSelection());
        ...
        float coerceIn2 = RangesKt.coerceIn(m2390getXimpl, Math.min(horizontalPosition, horizontalPosition2), Math.max(horizontalPosition, horizontalPosition2));
        if (Math.abs(m2390getXimpl - coerceIn2) > IntSize.m5033getWidthimpl(j) / 2) {
            return Offset.INSTANCE.m2405getUnspecifiedF1C5BW0();
        }
        return layoutCoordinates.mo3874localPositionOfR5De75A(innerTextFieldCoordinates, OffsetKt.Offset(coerceIn2, Offset.m2391getYimpl(m2420getCenterF1C5BW0)));
    }
}
