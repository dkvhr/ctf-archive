...
public final class ContentInViewModifier implements BringIntoViewResponder, OnRemeasuredModifier, OnPlacedModifier {
    ...
    public ContentInViewModifier(CoroutineScope scope, Orientation orientation, ScrollableState scrollState, boolean z) {
        ...
        this.viewportSize = IntSize.INSTANCE.m5038getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        ...
        if (!(!IntSize.m5031equalsimpl0(this.viewportSize, IntSize.INSTANCE.m5038getZeroYbymL2g()))) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.".toString());
        }
        return m246computeDestinationO0kMr_c(localRect, this.viewportSize);
    }

    @Override // androidx.compose.foundation.relocation.BringIntoViewResponder
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        Rect invoke = function0.invoke();
        if (invoke == null || m248isMaxVisibleO0kMr_c$default(this, invoke, 0L, 1, null)) {
            return Unit.INSTANCE;
        }
    ...
    public void mo250onRemeasuredozmzZPI(long size) {
        Rect focusedChildBounds;
        long j = this.viewportSize;
        this.viewportSize = size;
        if (m244compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m247isMaxVisibleO0kMr_c(rect, j) && !m247isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
    ...
    public final float calculateScrollDelta() {
        ...
        long m5043toSizeozmzZPI = IntSizeKt.m5043toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return relocationDistance(findBringIntoViewRequest.getTop(), findBringIntoViewRequest.getBottom(), Size.m2456getHeightimpl(m5043toSizeozmzZPI));
        }
        if (i == 2) {
            return relocationDistance(findBringIntoViewRequest.getLeft(), findBringIntoViewRequest.getRight(), Size.m2459getWidthimpl(m5043toSizeozmzZPI));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: computeDestination-O0kMr_c, reason: not valid java name */
    private final Rect m246computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m2427translatek4lQ0M(Offset.m2399unaryMinusF1C5BW0(m249relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    public static /* synthetic */ boolean m248isMaxVisibleO0kMr_c$default(ContentInViewModifier contentInViewModifier, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewModifier.viewportSize;
        }
        return contentInViewModifier.m247isMaxVisibleO0kMr_c(rect, j);
    }

    /* renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m247isMaxVisibleO0kMr_c(Rect rect, long j) {
        return Offset.m2387equalsimpl0(m249relocationOffsetBMxPBkI(rect, j), Offset.INSTANCE.m2406getZeroF1C5BW0());
    }

    /* renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m249relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long m5043toSizeozmzZPI = IntSizeKt.m5043toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, relocationDistance(childBounds.getTop(), childBounds.getBottom(), Size.m2456getHeightimpl(m5043toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(relocationDistance(childBounds.getLeft(), childBounds.getRight(), Size.m2459getWidthimpl(m5043toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }
    ...
    private final int m244compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m5032getHeightimpl(j), IntSize.m5032getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m5033getWidthimpl(j), IntSize.m5033getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m245compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m2456getHeightimpl(j), Size.m2456getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m2459getWidthimpl(j), Size.m2459getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (invoke != null) {
                    if (m245compareToiLBOSCw(invoke.m2423getSizeNHjbRc(), IntSizeKt.m5043toSizeozmzZPI(this.viewportSize)) > 0) {
                        return rect;
                    }
                    rect = invoke;
                }
