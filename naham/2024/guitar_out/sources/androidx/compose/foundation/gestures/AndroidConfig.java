...
public final class AndroidConfig implements ScrollConfig {
    ...
    public long mo243calculateMouseWheelScroll8xgXZGE(Density calculateMouseWheelScroll, PointerEvent event, long j) {
        Intrinsics.checkNotNullParameter(calculateMouseWheelScroll, "$this$calculateMouseWheelScroll");
        Intrinsics.checkNotNullParameter(event, "event");
        List<PointerInputChange> changes = event.getChanges();
        Offset m2379boximpl = Offset.m2379boximpl(Offset.INSTANCE.m2406getZeroF1C5BW0());
        int size = changes.size();
        for (int i = 0; i < size; i++) {
            m2379boximpl = Offset.m2379boximpl(Offset.m2395plusMKHz9U(m2379boximpl.getPackedValue(), changes.get(i).getScrollDelta()));
        }
        return Offset.m2397timestuRUvjQ(m2379boximpl.getPackedValue(), -calculateMouseWheelScroll.mo325toPx0680j_4(Dp.m4873constructorimpl(64)));
    }
}
