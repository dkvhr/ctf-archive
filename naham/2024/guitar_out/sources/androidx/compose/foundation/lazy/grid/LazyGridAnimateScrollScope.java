...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (this.state.getIsVertical()) {
            m4991getXimpl = IntOffset.m4992getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            m4991getXimpl = IntOffset.m4991getXimpl(lazyGridItemInfo2.getOffset());
        }
        return Integer.valueOf(m4991getXimpl);
    }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (intValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == intValue) {
                    if (isVertical) {
                        m5033getWidthimpl = IntSize.m5032getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        m5033getWidthimpl = IntSize.m5033getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    i4 = Math.max(i4, m5033getWidthimpl);
                    i++;
                }
                i2 += i4;
                i3++;
