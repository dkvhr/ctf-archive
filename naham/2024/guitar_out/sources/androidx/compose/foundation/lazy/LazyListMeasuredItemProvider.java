...
public abstract class LazyListMeasuredItemProvider {
    ...
    private LazyListMeasuredItemProvider(long j, boolean z, LazyListItemProvider itemProvider, LazyLayoutMeasureScope measureScope) {
        ...
        this.childConstraints = ConstraintsKt.Constraints$default(0, z ? Constraints.m4829getMaxWidthimpl(j) : Integer.MAX_VALUE, 0, !z ? Constraints.m4828getMaxHeightimpl(j) : Integer.MAX_VALUE, 5, null);
    }

    public final LazyListMeasuredItem getAndMeasure(int index) {
        return createItem(index, this.itemProvider.getKey(index), this.itemProvider.getContentType(index), this.measureScope.mo638measure0kLqBqw(index, this.childConstraints));
    }
