...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m620childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span == 1) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m4839fixedWidthOenEA2s(coerceAtLeast);
        }
        return Constraints.INSTANCE.m4838fixedHeightOenEA2s(coerceAtLeast);
    }

    /* renamed from: itemConstraints-OenEA2s, reason: not valid java name */
    public final long m621itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        return m620childConstraintsJhjzzOo$foundation_release(0, lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine()));
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            int m599getCurrentLineSpanimpl = GridItemSpan.m599getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i3).getPackedValue());
            LazyGridMeasuredItem m619getAndMeasure3p2s80s = this.measuredItemProvider.m619getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i3, i, m620childConstraintsJhjzzOo$foundation_release(i2, m599getCurrentLineSpanimpl));
            i2 += m599getCurrentLineSpanimpl;
            Unit unit = Unit.INSTANCE;
            lazyGridMeasuredItemArr[i3] = m619getAndMeasure3p2s80s;
        }
