...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo2486concat58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        if (MatrixKt.m2878isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m2501setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo2485clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m2494toRegionOp7u2Bmg(clipOp));
    }

    /* renamed from: toRegionOp--7u2Bmg, reason: not valid java name */
    public final Region.Op m2494toRegionOp7u2Bmg(int i) {
        return ClipOp.m2613equalsimpl0(i, ClipOp.INSTANCE.m2617getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo2490drawLineWko1d7g(long p1, long p2, Paint paint) {
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawLine(Offset.m2390getXimpl(p1), Offset.m2391getYimpl(p1), Offset.m2390getXimpl(p2), Offset.m2391getYimpl(p2), paint.asFrameworkPaint());
    }
    ...
    public void mo2487drawCircle9KIMszo(long center, float radius, Paint paint) {
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawCircle(Offset.m2390getXimpl(center), Offset.m2391getYimpl(center), radius, paint.asFrameworkPaint());
    }
    ...
    public void mo2488drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        Intrinsics.checkNotNullParameter(image, "image");
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m2390getXimpl(topLeftOffset), Offset.m2391getYimpl(topLeftOffset), paint.asFrameworkPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo2489drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        Intrinsics.checkNotNullParameter(image, "image");
        Intrinsics.checkNotNullParameter(paint, "paint");
        android.graphics.Canvas canvas = this.internalCanvas;
        Bitmap asAndroidBitmap = AndroidImageBitmap_androidKt.asAndroidBitmap(image);
        Rect rect = this.srcRect;
        rect.left = IntOffset.m4991getXimpl(srcOffset);
        rect.top = IntOffset.m4992getYimpl(srcOffset);
        rect.right = IntOffset.m4991getXimpl(srcOffset) + IntSize.m5033getWidthimpl(srcSize);
        rect.bottom = IntOffset.m4992getYimpl(srcOffset) + IntSize.m5032getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        rect2.left = IntOffset.m4991getXimpl(dstOffset);
        rect2.top = IntOffset.m4992getYimpl(dstOffset);
        rect2.right = IntOffset.m4991getXimpl(dstOffset) + IntSize.m5033getWidthimpl(dstSize);
        rect2.bottom = IntOffset.m4992getYimpl(dstOffset) + IntSize.m5032getHeightimpl(dstSize);
    ...
    public void mo2491drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        Intrinsics.checkNotNullParameter(points, "points");
        Intrinsics.checkNotNullParameter(paint, "paint");
        if (PointMode.m2923equalsimpl0(pointMode, PointMode.INSTANCE.m2927getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m2923equalsimpl0(pointMode, PointMode.INSTANCE.m2929getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m2923equalsimpl0(pointMode, PointMode.INSTANCE.m2928getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                ...
                this.internalCanvas.drawLine(Offset.m2390getXimpl(packedValue), Offset.m2391getYimpl(packedValue), Offset.m2390getXimpl(packedValue2), Offset.m2391getYimpl(packedValue2), asFrameworkPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo2492drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
        Intrinsics.checkNotNullParameter(points, "points");
        Intrinsics.checkNotNullParameter(paint, "paint");
    ...
    public void mo2493drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        Intrinsics.checkNotNullParameter(vertices, "vertices");
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m2537toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.asFrameworkPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo2484clipPathmtrdDE(Path path, int clipOp) {
        Intrinsics.checkNotNullParameter(path, "path");
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m2494toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long packedValue = points.get(i).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m2390getXimpl(packedValue), Offset.m2391getYimpl(packedValue), paint.asFrameworkPaint());
        }
    }
}
