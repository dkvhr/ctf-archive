...
public final class AndroidPath implements Path {
    ...
    public int mo2522getFillTypeRgk1Os() {
        if (this.internalPath.getFillType() == Path.FillType.EVEN_ODD) {
            return PathFillType.INSTANCE.m2905getEvenOddRgk1Os();
        }
        return PathFillType.INSTANCE.m2906getNonZeroRgk1Os();
    }

    @Override // androidx.compose.ui.graphics.Path
    /* renamed from: setFillType-oQ8Xj4U, reason: not valid java name */
    public void mo2524setFillTypeoQ8Xj4U(int i) {
        Path.FillType fillType;
        android.graphics.Path path = this.internalPath;
        if (PathFillType.m2901equalsimpl0(i, PathFillType.INSTANCE.m2905getEvenOddRgk1Os())) {
    ...
    public void addRoundRect(RoundRect roundRect) {
        ...
        this.radii[0] = CornerRadius.m2365getXimpl(roundRect.m2440getTopLeftCornerRadiuskKHJgLs());
        this.radii[1] = CornerRadius.m2366getYimpl(roundRect.m2440getTopLeftCornerRadiuskKHJgLs());
        this.radii[2] = CornerRadius.m2365getXimpl(roundRect.m2441getTopRightCornerRadiuskKHJgLs());
        this.radii[3] = CornerRadius.m2366getYimpl(roundRect.m2441getTopRightCornerRadiuskKHJgLs());
        this.radii[4] = CornerRadius.m2365getXimpl(roundRect.m2439getBottomRightCornerRadiuskKHJgLs());
        this.radii[5] = CornerRadius.m2366getYimpl(roundRect.m2439getBottomRightCornerRadiuskKHJgLs());
        this.radii[6] = CornerRadius.m2365getXimpl(roundRect.m2438getBottomLeftCornerRadiuskKHJgLs());
        this.radii[7] = CornerRadius.m2366getYimpl(roundRect.m2438getBottomLeftCornerRadiuskKHJgLs());
    ...
    public void mo2526translatek4lQ0M(long offset) {
        this.mMatrix.reset();
        this.mMatrix.setTranslate(Offset.m2390getXimpl(offset), Offset.m2391getYimpl(offset));
        this.internalPath.transform(this.mMatrix);
    }

    @Override // androidx.compose.ui.graphics.Path
    /* renamed from: transform-58bKbWc, reason: not valid java name */
    public void mo2525transform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        AndroidMatrixConversions_androidKt.m2501setFromEL8BTi8(this.mMatrix, matrix);
        this.internalPath.transform(this.mMatrix);
    }
    ...
    public boolean mo2523opN5in7k0(Path path1, Path path2, int operation) {
        Path.Op op;
        Intrinsics.checkNotNullParameter(path1, "path1");
        Intrinsics.checkNotNullParameter(path2, "path2");
        if (PathOperation.m2910equalsimpl0(operation, PathOperation.INSTANCE.m2914getDifferenceb3I0S0c())) {
            op = Path.Op.DIFFERENCE;
        } else if (PathOperation.m2910equalsimpl0(operation, PathOperation.INSTANCE.m2915getIntersectb3I0S0c())) {
            op = Path.Op.INTERSECT;
        } else if (PathOperation.m2910equalsimpl0(operation, PathOperation.INSTANCE.m2916getReverseDifferenceb3I0S0c())) {
            op = Path.Op.REVERSE_DIFFERENCE;
        } else {
            op = PathOperation.m2910equalsimpl0(operation, PathOperation.INSTANCE.m2917getUnionb3I0S0c()) ? Path.Op.UNION : Path.Op.XOR;
        }
        android.graphics.Path path = this.internalPath;
    ...
    public void mo2521addPathUv8p0NA(Path path, long offset) {
        Intrinsics.checkNotNullParameter(path, "path");
        android.graphics.Path path2 = this.internalPath;
        if (path instanceof AndroidPath) {
            path2.addPath(((AndroidPath) path).getInternalPath(), Offset.m2390getXimpl(offset), Offset.m2391getYimpl(offset));
            return;
        }
        throw new UnsupportedOperationException("Unable to obtain android.graphics.Path");
    }
