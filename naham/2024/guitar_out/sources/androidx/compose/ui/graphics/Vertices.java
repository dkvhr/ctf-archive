...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m2683toArgb8_81llA(colors.get(i).m2639unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                m2391getYimpl = Offset.m2390getXimpl(packedValue);
            } else {
                m2391getYimpl = Offset.m2391getYimpl(packedValue);
            }
            fArr[i] = m2391getYimpl;
        }
        return fArr;
    }
}
