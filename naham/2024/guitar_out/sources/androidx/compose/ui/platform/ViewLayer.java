...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView ownerView, DrawChildContainer container, Function1<? super Canvas, Unit> drawBlock, Function0<Unit> invalidateParentLayer) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m3013getCenterSzJe1aQ();
    ...
    public void mo4132updateLayerPropertiesdDxrwY(float scaleX, float scaleY, float alpha, float translationX, float translationY, float shadowElevation, float rotationX, float rotationY, float rotationZ, float cameraDistance, long transformOrigin, Shape shape, boolean clip, RenderEffect renderEffect, long ambientShadowColor, long spotShadowColor, int compositingStrategy, LayoutDirection layoutDirection, Density density) {
        ...
        setPivotX(TransformOrigin.m3008getPivotFractionXimpl(this.mTransformOrigin) * getWidth());
        setPivotY(TransformOrigin.m3009getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(viewLayer, ColorKt.m2683toArgb8_81llA(ambientShadowColor));
            ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(viewLayer, ColorKt.m2683toArgb8_81llA(spotShadowColor));
        }
        if (Build.VERSION.SDK_INT >= 31) {
            ViewLayerVerificationHelper31.INSTANCE.setRenderEffect(this, renderEffect);
        }
        if (CompositingStrategy.m2710equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m2716getOffscreenNrFUSI())) {
            setLayerType(2, null);
        } else if (CompositingStrategy.m2710equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m2715getModulateAlphaNrFUSI())) {
            setLayerType(0, null);
            z = false;
        } else {
            setLayerType(0, null);
    ...
    public boolean mo4127isInLayerk4lQ0M(long position) {
        ...
        if (this.clipToBounds) {
            return 0.0f <= m2390getXimpl && m2390getXimpl < ((float) getWidth()) && 0.0f <= m2391getYimpl && m2391getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
    ...
    public void mo4130resizeozmzZPI(long size) {
        ...
        if (m5033getWidthimpl == getWidth() && m5032getHeightimpl == getHeight()) {
            return;
        }
        float f = m5033getWidthimpl;
        setPivotX(TransformOrigin.m3008getPivotFractionXimpl(this.mTransformOrigin) * f);
        float f2 = m5032getHeightimpl;
        setPivotY(TransformOrigin.m3009getPivotFractionYimpl(this.mTransformOrigin) * f2);
        this.outlineResolver.m4221updateuvyYCjk(SizeKt.Size(f, f2));
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + m5033getWidthimpl, getTop() + m5032getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo4129movegyyYBs(long position) {
        ...
        if (m4991getXimpl != getLeft()) {
            offsetLeftAndRight(m4991getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int m4992getYimpl = IntOffset.m4992getYimpl(position);
        if (m4992getYimpl != getTop()) {
            offsetTopAndBottom(m4992getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo4128mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] m4197calculateInverseMatrixbWbORWo = this.matrixCache.m4197calculateInverseMatrixbWbORWo(this);
        ...
        return androidx.compose.ui.graphics.Matrix.m2860mapMKHz9U(this.matrixCache.m4198calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        ...
        androidx.compose.ui.graphics.Matrix.m2862mapimpl(this.matrixCache.m4198calculateMatrixGrdbGEg(this), rect);
    }
    ...
    public void mo4131transform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        androidx.compose.ui.graphics.Matrix.m2871timesAssign58bKbWc(matrix, this.matrixCache.m4198calculateMatrixGrdbGEg(this));
    }
