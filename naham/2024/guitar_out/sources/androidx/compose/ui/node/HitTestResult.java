...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long m3999findBestHitDistanceptXAw2c = m3999findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m3994getDistanceimpl(m3999findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m3996isInLayerimpl(m3999findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m3990compareToS_HNhKs(m3999findBestHitDistanceptXAw2c(), DistanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m3999findBestHitDistanceptXAw2c() {
        long DistanceAndInLayer;
        ...
        if (i <= lastIndex) {
            while (true) {
                long m3991constructorimpl = DistanceAndInLayer.m3991constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m3990compareToS_HNhKs(m3991constructorimpl, DistanceAndInLayer) < 0) {
                    DistanceAndInLayer = m3991constructorimpl;
                }
                if (DistanceAndInLayer.m3994getDistanceimpl(DistanceAndInLayer) < 0.0f && DistanceAndInLayer.m3996isInLayerimpl(DistanceAndInLayer)) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m3990compareToS_HNhKs(m3999findBestHitDistanceptXAw2c, m3999findBestHitDistanceptXAw2c()) > 0) {
