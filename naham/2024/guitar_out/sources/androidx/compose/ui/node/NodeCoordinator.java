...
public abstract class NodeCoordinator extends LookaheadCapablePlaceable implements Measurable, LayoutCoordinates, OwnerScope, Function1<Canvas, Unit> {
    ...
    private static final float[] tmpMatrix = Matrix.m2854constructorimpl$default(null, 1, null);
    private static final HitTestSource PointerInputSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$PointerInputSource$1
        ...
        public void mo4076childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            Intrinsics.checkNotNullParameter(layoutNode, "layoutNode");
            Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
            layoutNode.m4006hitTestM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw, reason: not valid java name */
        public int mo4077entityTypeOLwlOKw() {
            return NodeKind.m4081constructorimpl(16);
        }
        ...
        public boolean interceptOutOfBoundsChildEvents(Modifier.Node node) {
            ...
            int m4081constructorimpl = NodeKind.m4081constructorimpl(16);
            ...
            while (node != 0) {
                ...
                } else if ((node.getKindSet() & m4081constructorimpl) != 0 && (node instanceof DelegatingNode)) {
    ...
    private static final HitTestSource SemanticsSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$SemanticsSource$1
        ...
        public void mo4076childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            Intrinsics.checkNotNullParameter(layoutNode, "layoutNode");
            Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
            layoutNode.m4007hitTestSemanticsM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw */
        public int mo4077entityTypeOLwlOKw() {
            return NodeKind.m4081constructorimpl(8);
        }
    };
    ...
    public interface HitTestSource {
        /* renamed from: childHitTest-YqVAtuI */
        void mo4076childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer);

        /* renamed from: entityType-OLwlOKw */
        int mo4077entityTypeOLwlOKw();
    ...
    protected void m4072setPositiongyyYBs(long j) {
        this.position = j;
    }
    ...
    public NodeCoordinator(LayoutNode layoutNode) {
        ...
        this.position = IntOffset.INSTANCE.m5001getZeronOccac();
    ...
    public final /* synthetic */ <T> void m4074visitNodesaLcG6gQ(int type, Function1<? super T, Unit> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        boolean m4090getIncludeSelfInTraversalH91voCI = NodeKindKt.m4090getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m4090getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return;
        }
        for (Modifier.Node headNode = headNode(m4090getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & type) != 0; headNode = headNode.getChild()) {
    ...
    private final boolean m4054hasNodeH91voCI(int type) {
        Modifier.Node headNode = headNode(NodeKindKt.m4090getIncludeSelfInTraversalH91voCI(type));
        return headNode != null && DelegatableNodeKt.m3977has64DMado(headNode, type);
    }

    /* renamed from: head-H91voCI */
    public final Modifier.Node m4067headH91voCI(int type) {
        boolean m4090getIncludeSelfInTraversalH91voCI = NodeKindKt.m4090getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m4090getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return null;
    ...
    public final long mo3873getSizeYbymL2g() {
    ...
    public void replace$ui_release() {
        mo3867placeAtf8xVGno(getPosition(), this.zIndex, this.layerBlock);
    }
    ...
    protected void onMeasureResultChanged(int width, int height) {
        ...
        if (ownedLayer != null) {
            ownedLayer.mo4130resizeozmzZPI(IntSizeKt.IntSize(width, height));
        } else {
            NodeCoordinator nodeCoordinator = this.wrappedBy;
            if (nodeCoordinator != null) {
        ...
        m3921setMeasuredSizeozmzZPI(IntSizeKt.IntSize(width, height));
        updateLayerParameters(false);
        int m4081constructorimpl = NodeKind.m4081constructorimpl(4);
        boolean m4090getIncludeSelfInTraversalH91voCI = NodeKindKt.m4090getIncludeSelfInTraversalH91voCI(m4081constructorimpl);
        Modifier.Node tail = getTail();
        if (m4090getIncludeSelfInTraversalH91voCI || (tail = tail.getParent()) != null) {
            for (Modifier.Node headNode = headNode(m4090getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & m4081constructorimpl) != 0; headNode = headNode.getChild()) {
                if ((headNode.getKindSet() & m4081constructorimpl) != 0) {
    ...
    public Object getParentData() {
        if (!getLayoutNode().getNodes().m4044hasH91voCI$ui_release(NodeKind.m4081constructorimpl(64))) {
            return null;
        }
        getTail();
        Ref.ObjectRef objectRef = new Ref.ObjectRef();
        for (Modifier.Node tail = getLayoutNode().getNodes().getTail(); tail != null; tail = tail.getParent()) {
            if ((NodeKind.m4081constructorimpl(64) & tail.getKindSet()) != 0) {
                int m4081constructorimpl = NodeKind.m4081constructorimpl(64);
                MutableVector mutableVector = null;
    ...
    public final long m4065getLastMeasurementConstraintsmsEJaDk$ui_release() {
        return getMeasurementConstraints();
    }

    /* renamed from: performingMeasure-K40F9xA */
    protected final Placeable m4070performingMeasureK40F9xA(long constraints, Function0<? extends Placeable> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        m3922setMeasurementConstraintsBRTryo0(constraints);
        return block.invoke();
    }

    @Override // androidx.compose.ui.layout.Placeable
    /* renamed from: placeAt-f8xVGno */
    public void mo3867placeAtf8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        m4058placeSelff8xVGno(position, zIndex, layerBlock);
    }

    /* renamed from: placeSelf-f8xVGno */
    private final void m4058placeSelff8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        updateLayerBlock$default(this, layerBlock, false, 2, null);
        if (!IntOffset.m4990equalsimpl0(getPosition(), position)) {
            m4072setPositiongyyYBs(position);
            getLayoutNode().getLayoutDelegate().getMeasurePassDelegate().notifyChildrenUsingCoordinatesWhilePlacing();
            OwnedLayer ownedLayer = this.layer;
            if (ownedLayer != null) {
                ownedLayer.mo4129movegyyYBs(position);
            } else {
                NodeCoordinator nodeCoordinator = this.wrappedBy;
                if (nodeCoordinator != null) {
                    nodeCoordinator.invalidateLayer();
    ...
    public final void m4071placeSelfApparentToRealOffsetf8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        long j = getApparentToRealOffset();
        m4058placeSelff8xVGno(IntOffsetKt.IntOffset(IntOffset.m4991getXimpl(position) + IntOffset.m4991getXimpl(j), IntOffset.m4992getYimpl(position) + IntOffset.m4992getYimpl(j)), zIndex, layerBlock);
    }
    ...
    public final void draw(Canvas canvas) {
        ...
        float m4991getXimpl = IntOffset.m4991getXimpl(getPosition());
        float m4992getYimpl = IntOffset.m4992getYimpl(getPosition());
        canvas.translate(m4991getXimpl, m4992getYimpl);
    ...
    public final void updateLayerBlock(Function1<? super GraphicsLayerScope, Unit> layerBlock, boolean forceUpdateLayerParameters) {
        ...
        createLayer.mo4130resizeozmzZPI(getMeasuredSize());
        createLayer.mo4129movegyyYBs(getPosition());
    ...
    private final void updateLayerParameters(boolean invokeOnLayoutChange) {
        ...
        reusableGraphicsLayerScope.m2934setSizeuvyYCjk(IntSizeKt.m5043toSizeozmzZPI(mo3873getSizeYbymL2g()));
    ...
    public final long m4066getMinimumTouchTargetSizeNHjbRc() {
        return this.layerDensity.mo326toSizeXkaWNTQ(getLayoutNode().getViewConfiguration().mo4010getMinimumTouchTargetSizeMYxV2XQ());
    }

    /* renamed from: hitTest-YqVAtuI */
    public final void m4068hitTestYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        Intrinsics.checkNotNullParameter(hitTestSource, "hitTestSource");
        Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
        Modifier.Node m4067headH91voCI = m4067headH91voCI(hitTestSource.mo4077entityTypeOLwlOKw());
        if (!m4075withinLayerBoundsk4lQ0M(pointerPosition)) {
            if (isTouchEvent) {
                float m4063distanceInMinimumTouchTargettz77jQw = m4063distanceInMinimumTouchTargettz77jQw(pointerPosition, m4066getMinimumTouchTargetSizeNHjbRc());
                if (Float.isInfinite(m4063distanceInMinimumTouchTargettz77jQw) || Float.isNaN(m4063distanceInMinimumTouchTargettz77jQw) || !hitTestResult.isHitInMinimumTouchTargetBetter(m4063distanceInMinimumTouchTargettz77jQw, false)) {
                    return;
                }
                m4056hitNearJHbHoSQ(m4067headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, false, m4063distanceInMinimumTouchTargettz77jQw);
                return;
            }
            return;
        }
        if (m4067headH91voCI == null) {
            mo4000hitTestChildYqVAtuI(hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        if (m4069isPointerInBoundsk4lQ0M(pointerPosition)) {
            m4055hit1hIXUjU(m4067headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        float m4063distanceInMinimumTouchTargettz77jQw2 = !isTouchEvent ? Float.POSITIVE_INFINITY : m4063distanceInMinimumTouchTargettz77jQw(pointerPosition, m4066getMinimumTouchTargetSizeNHjbRc());
        if (!Float.isInfinite(m4063distanceInMinimumTouchTargettz77jQw2) && !Float.isNaN(m4063distanceInMinimumTouchTargettz77jQw2)) {
            if (hitTestResult.isHitInMinimumTouchTargetBetter(m4063distanceInMinimumTouchTargettz77jQw2, isInLayer)) {
                m4056hitNearJHbHoSQ(m4067headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m4063distanceInMinimumTouchTargettz77jQw2);
                return;
            }
        }
        m4059speculativeHitJHbHoSQ(m4067headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m4063distanceInMinimumTouchTargettz77jQw2);
    }

    /* renamed from: hit-1hIXUjU */
    public final void m4055hit1hIXUjU(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2) {
        if (node == null) {
            mo4000hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hit(node, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m4079nextUntilhw7D004;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4079nextUntilhw7D004 = NodeCoordinatorKt.m4079nextUntilhw7D004(node, hitTestSource.mo4077entityTypeOLwlOKw(), NodeKind.m4081constructorimpl(2));
                    nodeCoordinator.m4055hit1hIXUjU(m4079nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2);
                }
            });
        }
    }
    ...
    public final void m4056hitNearJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        if (node == null) {
            mo4000hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hitInMinimumTouchTarget(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hitNear$1
                ...
                public final void invoke2() {
                    ...
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4079nextUntilhw7D004 = NodeCoordinatorKt.m4079nextUntilhw7D004(node, hitTestSource.mo4077entityTypeOLwlOKw(), NodeKind.m4081constructorimpl(2));
                    nodeCoordinator.m4056hitNearJHbHoSQ(m4079nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public final void m4059speculativeHitJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        Modifier.Node m4079nextUntilhw7D004;
        if (node == null) {
            mo4000hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else if (!hitTestSource.interceptOutOfBoundsChildEvents(node)) {
            m4079nextUntilhw7D004 = NodeCoordinatorKt.m4079nextUntilhw7D004(node, hitTestSource.mo4077entityTypeOLwlOKw(), NodeKind.m4081constructorimpl(2));
            m4059speculativeHitJHbHoSQ(m4079nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
        } else {
            hitTestResult.speculativeHit(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$speculativeHit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m4079nextUntilhw7D0042;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4079nextUntilhw7D0042 = NodeCoordinatorKt.m4079nextUntilhw7D004(node, hitTestSource.mo4077entityTypeOLwlOKw(), NodeKind.m4081constructorimpl(2));
                    nodeCoordinator.m4059speculativeHitJHbHoSQ(m4079nextUntilhw7D0042, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public void mo4000hitTestChildYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        Intrinsics.checkNotNullParameter(hitTestSource, "hitTestSource");
        Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
        NodeCoordinator nodeCoordinator = this.wrapped;
        if (nodeCoordinator != null) {
            nodeCoordinator.m4068hitTestYqVAtuI(hitTestSource, nodeCoordinator.m4064fromParentPositionMKHz9U(pointerPosition), hitTestResult, isTouchEvent, isInLayer);
        }
    }
    ...
    public final Rect touchBoundsInRoot() {
        ...
        long m4062calculateMinimumTouchTargetPaddingE7KxVPU = m4062calculateMinimumTouchTargetPaddingE7KxVPU(m4066getMinimumTouchTargetSizeNHjbRc());
        rectCache.setLeft(-Size.m2459getWidthimpl(m4062calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setTop(-Size.m2456getHeightimpl(m4062calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setRight(getMeasuredWidth() + Size.m2459getWidthimpl(m4062calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setBottom(getMeasuredHeight() + Size.m2456getHeightimpl(m4062calculateMinimumTouchTargetPaddingE7KxVPU));
    ...
    public long mo3878windowToLocalMKHz9U(long relativeToWindow) {
        if (!isAttached()) {
            throw new IllegalStateException(ExpectAttachedLayoutCoordinates.toString());
        }
        LayoutCoordinates findRootCoordinates = LayoutCoordinatesKt.findRootCoordinates(this);
        return mo3874localPositionOfR5De75A(findRootCoordinates, Offset.m2394minusMKHz9U(LayoutNodeKt.requireOwner(getLayoutNode()).mo4133calculateLocalPositionMKHz9U(relativeToWindow), LayoutCoordinatesKt.positionInRoot(findRootCoordinates)));
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: localToWindow-MK-Hz9U */
    public long mo3876localToWindowMKHz9U(long relativeToLocal) {
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo4134calculatePositionInWindowMKHz9U(mo3875localToRootMKHz9U(relativeToLocal));
    }
    ...
    public long mo3874localPositionOfR5De75A(LayoutCoordinates sourceCoordinates, long relativeToSource) {
        Intrinsics.checkNotNullParameter(sourceCoordinates, "sourceCoordinates");
        if (sourceCoordinates instanceof LookaheadLayoutCoordinatesImpl) {
            return Offset.m2399unaryMinusF1C5BW0(sourceCoordinates.mo3874localPositionOfR5De75A(this, Offset.m2399unaryMinusF1C5BW0(relativeToSource)));
        }
        ...
        while (coordinator != findCommonAncestor$ui_release) {
            relativeToSource = coordinator.m4073toParentPositionMKHz9U(relativeToSource);
            coordinator = coordinator.wrappedBy;
            Intrinsics.checkNotNull(coordinator);
        }
        return m4053ancestorToLocalR5De75A(findCommonAncestor$ui_release, relativeToSource);
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: transformFrom-EL8BTi8 */
    public void mo3877transformFromEL8BTi8(LayoutCoordinates sourceCoordinates, float[] matrix) {
        Intrinsics.checkNotNullParameter(sourceCoordinates, "sourceCoordinates");
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        NodeCoordinator coordinator = toCoordinator(sourceCoordinates);
        coordinator.onCoordinatesUsed$ui_release();
        NodeCoordinator findCommonAncestor$ui_release = findCommonAncestor$ui_release(coordinator);
        Matrix.m2863resetimpl(matrix);
        coordinator.m4061transformToAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
        m4060transformFromAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
    }

    /* renamed from: transformToAncestor-EL8BTi8 */
    private final void m4061transformToAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        NodeCoordinator nodeCoordinator = this;
        while (!Intrinsics.areEqual(nodeCoordinator, ancestor)) {
            OwnedLayer ownedLayer = nodeCoordinator.layer;
            if (ownedLayer != null) {
                ownedLayer.mo4131transform58bKbWc(matrix);
            }
            if (!IntOffset.m4990equalsimpl0(nodeCoordinator.getPosition(), IntOffset.INSTANCE.m5001getZeronOccac())) {
                float[] fArr = tmpMatrix;
                Matrix.m2863resetimpl(fArr);
                Matrix.m2874translateimpl$default(fArr, IntOffset.m4991getXimpl(r1), IntOffset.m4992getYimpl(r1), 0.0f, 4, null);
                Matrix.m2871timesAssign58bKbWc(matrix, fArr);
            }
            nodeCoordinator = nodeCoordinator.wrappedBy;
            Intrinsics.checkNotNull(nodeCoordinator);
        }
    ...
    private final void m4060transformFromAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        if (Intrinsics.areEqual(ancestor, this)) {
            return;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        Intrinsics.checkNotNull(nodeCoordinator);
        nodeCoordinator.m4060transformFromAncestorEL8BTi8(ancestor, matrix);
        if (!IntOffset.m4990equalsimpl0(getPosition(), IntOffset.INSTANCE.m5001getZeronOccac())) {
            float[] fArr = tmpMatrix;
            Matrix.m2863resetimpl(fArr);
            Matrix.m2874translateimpl$default(fArr, -IntOffset.m4991getXimpl(getPosition()), -IntOffset.m4992getYimpl(getPosition()), 0.0f, 4, null);
            Matrix.m2871timesAssign58bKbWc(matrix, fArr);
        }
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            ownedLayer.mo4126inverseTransform58bKbWc(matrix);
        }
    }
    ...
    public Rect localBoundingBoxOf(LayoutCoordinates sourceCoordinates, boolean clipBounds) {
        ...
        rectCache.setRight(IntSize.m5033getWidthimpl(sourceCoordinates.mo3873getSizeYbymL2g()));
        rectCache.setBottom(IntSize.m5032getHeightimpl(sourceCoordinates.mo3873getSizeYbymL2g()));
        while (coordinator != findCommonAncestor$ui_release) {
    ...
    private final long m4053ancestorToLocalR5De75A(NodeCoordinator ancestor, long offset) {
        if (ancestor == this) {
            return offset;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        if (nodeCoordinator == null || Intrinsics.areEqual(ancestor, nodeCoordinator)) {
            return m4064fromParentPositionMKHz9U(offset);
        }
        return m4064fromParentPositionMKHz9U(nodeCoordinator.m4053ancestorToLocalR5De75A(ancestor, offset));
    }
    ...
    public long mo3875localToRootMKHz9U(long relativeToLocal) {
        if (!isAttached()) {
            throw new IllegalStateException(ExpectAttachedLayoutCoordinates.toString());
        }
        onCoordinatesUsed$ui_release();
        for (NodeCoordinator nodeCoordinator = this; nodeCoordinator != null; nodeCoordinator = nodeCoordinator.wrappedBy) {
            relativeToLocal = nodeCoordinator.m4073toParentPositionMKHz9U(relativeToLocal);
        }
    ...
    public long m4073toParentPositionMKHz9U(long position) {
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            position = ownedLayer.mo4128mapOffset8S9VItk(position, false);
        }
        return IntOffsetKt.m5005plusNvtHpc(position, getPosition());
    }

    /* renamed from: fromParentPosition-MK-Hz9U */
    public long m4064fromParentPositionMKHz9U(long position) {
        long m5003minusNvtHpc = IntOffsetKt.m5003minusNvtHpc(position, getPosition());
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer != null ? ownedLayer.mo4128mapOffset8S9VItk(m5003minusNvtHpc, true) : m5003minusNvtHpc;
    }

    public final void drawBorder(Canvas canvas, Paint paint) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(paint, "paint");
        canvas.drawRect(new Rect(0.5f, 0.5f, IntSize.m5033getWidthimpl(getMeasuredSize()) - 0.5f, IntSize.m5032getHeightimpl(getMeasuredSize()) - 0.5f), paint);
    }
    ...
    public final void rectInParent$ui_release(MutableRect bounds, boolean clipBounds, boolean clipToMinimumTouchTargetSize) {
        ...
        if (ownedLayer != null) {
            if (this.isClipping) {
                if (clipToMinimumTouchTargetSize) {
                    long m4066getMinimumTouchTargetSizeNHjbRc = m4066getMinimumTouchTargetSizeNHjbRc();
                    float m2459getWidthimpl = Size.m2459getWidthimpl(m4066getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    float m2456getHeightimpl = Size.m2456getHeightimpl(m4066getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    bounds.intersect(-m2459getWidthimpl, -m2456getHeightimpl, IntSize.m5033getWidthimpl(mo3873getSizeYbymL2g()) + m2459getWidthimpl, IntSize.m5032getHeightimpl(mo3873getSizeYbymL2g()) + m2456getHeightimpl);
                } else if (clipBounds) {
                    bounds.intersect(0.0f, 0.0f, IntSize.m5033getWidthimpl(mo3873getSizeYbymL2g()), IntSize.m5032getHeightimpl(mo3873getSizeYbymL2g()));
                }
                if (bounds.isEmpty()) {
                    return;
                }
        ...
        float m4991getXimpl = IntOffset.m4991getXimpl(getPosition());
        bounds.setLeft(bounds.getLeft() + m4991getXimpl);
        bounds.setRight(bounds.getRight() + m4991getXimpl);
        float m4992getYimpl = IntOffset.m4992getYimpl(getPosition());
        bounds.setTop(bounds.getTop() + m4992getYimpl);
        bounds.setBottom(bounds.getBottom() + m4992getYimpl);
    }
    ...
    public final boolean m4075withinLayerBoundsk4lQ0M(long pointerPosition) {
        if (!OffsetKt.m2407isFinitek4lQ0M(pointerPosition)) {
            return false;
        }
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer == null || !this.isClipping || ownedLayer.mo4127isInLayerk4lQ0M(pointerPosition);
    }

    /* renamed from: isPointerInBounds-k-4lQ0M */
    protected final boolean m4069isPointerInBoundsk4lQ0M(long pointerPosition) {
        float m2390getXimpl = Offset.m2390getXimpl(pointerPosition);
        float m2391getYimpl = Offset.m2391getYimpl(pointerPosition);
        return m2390getXimpl >= 0.0f && m2391getYimpl >= 0.0f && m2390getXimpl < ((float) getMeasuredWidth()) && m2391getYimpl < ((float) getMeasuredHeight());
    }
    ...
    static /* synthetic */ Object propagateRelocationRequest$suspendImpl(NodeCoordinator nodeCoordinator, Rect rect, Continuation<? super Unit> continuation) {
        ...
        return (nodeCoordinator2 != null && (propagateRelocationRequest = nodeCoordinator2.propagateRelocationRequest(rect.m2427translatek4lQ0M(nodeCoordinator2.localBoundingBoxOf(nodeCoordinator, false).m2425getTopLeftF1C5BW0()), continuation)) == IntrinsicsKt.getCOROUTINE_SUSPENDED()) ? propagateRelocationRequest : Unit.INSTANCE;
    }
    ...
    public final NodeCoordinator findCommonAncestor$ui_release(NodeCoordinator other) {
        ...
        if (layoutNode == layoutNode2) {
            ...
            int m4081constructorimpl = NodeKind.m4081constructorimpl(2);
            if (!tail2.getNode().getIsAttached()) {
                throw new IllegalStateException("visitLocalAncestors called on an unattached node".toString());
            }
            for (Modifier.Node parent = tail2.getNode().getParent(); parent != null; parent = parent.getParent()) {
                if ((parent.getKindSet() & m4081constructorimpl) != 0 && parent == tail) {
                    return other;
                }
            }
            return this;
    ...
    private final long m4057offsetFromEdgeMKHz9U(long pointerPosition) {
        float m2390getXimpl = Offset.m2390getXimpl(pointerPosition);
        float max = Math.max(0.0f, m2390getXimpl < 0.0f ? -m2390getXimpl : m2390getXimpl - getMeasuredWidth());
        float m2391getYimpl = Offset.m2391getYimpl(pointerPosition);
        return OffsetKt.Offset(max, Math.max(0.0f, m2391getYimpl < 0.0f ? -m2391getYimpl : m2391getYimpl - getMeasuredHeight()));
    }

    /* renamed from: calculateMinimumTouchTargetPadding-E7KxVPU */
    protected final long m4062calculateMinimumTouchTargetPaddingE7KxVPU(long minimumTouchTargetSize) {
        return SizeKt.Size(Math.max(0.0f, (Size.m2459getWidthimpl(minimumTouchTargetSize) - getMeasuredWidth()) / 2.0f), Math.max(0.0f, (Size.m2456getHeightimpl(minimumTouchTargetSize) - getMeasuredHeight()) / 2.0f));
    }

    /* renamed from: distanceInMinimumTouchTarget-tz77jQw */
    public final float m4063distanceInMinimumTouchTargettz77jQw(long pointerPosition, long minimumTouchTargetSize) {
        if (getMeasuredWidth() >= Size.m2459getWidthimpl(minimumTouchTargetSize) && getMeasuredHeight() >= Size.m2456getHeightimpl(minimumTouchTargetSize)) {
            return Float.POSITIVE_INFINITY;
        }
        long m4062calculateMinimumTouchTargetPaddingE7KxVPU = m4062calculateMinimumTouchTargetPaddingE7KxVPU(minimumTouchTargetSize);
        float m2459getWidthimpl = Size.m2459getWidthimpl(m4062calculateMinimumTouchTargetPaddingE7KxVPU);
        float m2456getHeightimpl = Size.m2456getHeightimpl(m4062calculateMinimumTouchTargetPaddingE7KxVPU);
        long m4057offsetFromEdgeMKHz9U = m4057offsetFromEdgeMKHz9U(pointerPosition);
        if ((m2459getWidthimpl > 0.0f || m2456getHeightimpl > 0.0f) && Offset.m2390getXimpl(m4057offsetFromEdgeMKHz9U) <= m2459getWidthimpl && Offset.m2391getYimpl(m4057offsetFromEdgeMKHz9U) <= m2456getHeightimpl) {
            return Offset.m2389getDistanceSquaredimpl(m4057offsetFromEdgeMKHz9U);
        }
        return Float.POSITIVE_INFINITY;
    }
    ...
    public final void onMeasured() {
        ...
        if (m4054hasNodeH91voCI(NodeKind.m4081constructorimpl(128))) {
            Snapshot createNonObservableSnapshot = Snapshot.INSTANCE.createNonObservableSnapshot();
            try {
                Snapshot makeCurrent = createNonObservableSnapshot.makeCurrent();
                try {
                    int m4081constructorimpl = NodeKind.m4081constructorimpl(128);
                    boolean m4090getIncludeSelfInTraversalH91voCI = NodeKindKt.m4090getIncludeSelfInTraversalH91voCI(m4081constructorimpl);
                    if (m4090getIncludeSelfInTraversalH91voCI) {
                        parent = getTail();
                    } else {
                        ...
                        if ((headNode.getKindSet() & m4081constructorimpl) != 0) {
                            ...
                            while (delegatingNode != 0) {
                                if (delegatingNode instanceof LayoutAwareModifierNode) {
                                    ((LayoutAwareModifierNode) delegatingNode).mo3964onRemeasuredozmzZPI(getMeasuredSize());
                                } else if ((delegatingNode.getKindSet() & m4081constructorimpl) != 0 && (delegatingNode instanceof DelegatingNode)) {
                                    Modifier.Node delegate = delegatingNode.getDelegate();
                                    int i = 0;
                                    delegatingNode = delegatingNode;
    ...
    public final void drawContainedDrawModifiers(Canvas canvas) {
        Modifier.Node m4067headH91voCI = m4067headH91voCI(NodeKind.m4081constructorimpl(4));
        if (m4067headH91voCI == null) {
            performDraw(canvas);
        } else {
            getLayoutNode().getMDrawScope$ui_release().m4016drawx_KDEd0$ui_release(canvas, IntSizeKt.m5043toSizeozmzZPI(mo3873getSizeYbymL2g()), this, m4067headH91voCI);
        }
    }
    ...
    public final boolean shouldSharePointerInputWithSiblings() {
        Modifier.Node headNode = headNode(NodeKindKt.m4090getIncludeSelfInTraversalH91voCI(NodeKind.m4081constructorimpl(16)));
        if (headNode == null) {
            return false;
        }
        Modifier.Node node = headNode;
        ...
        if ((node2.getAggregateChildKindSet() & m4081constructorimpl) != 0) {
            for (Modifier.Node child = node2.getChild(); child != null; child = child.getChild()) {
                if ((child.getKindSet() & m4081constructorimpl) != 0) {
