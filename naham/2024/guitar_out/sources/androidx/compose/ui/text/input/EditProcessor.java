...
public final class EditProcessor {
    ...
    private TextFieldValue mBufferState = new TextFieldValue(AnnotatedStringKt.emptyAnnotatedString(), TextRange.INSTANCE.m4409getZerod9O1mEE(), (TextRange) null, (DefaultConstructorMarker) null);
    ...
    public final void reset(TextFieldValue value, TextInputSession textInputSession) {
        ...
        boolean z = true;
        boolean z2 = !Intrinsics.areEqual(value.getComposition(), this.mBuffer.m4545getCompositionMzsxiRA$ui_text_release());
        boolean z3 = false;
        ...
        } else if (TextRange.m4397equalsimpl0(this.mBufferState.getSelection(), value.getSelection())) {
            z = false;
        } else {
            this.mBuffer.setSelection$ui_text_release(TextRange.m4402getMinimpl(value.getSelection()), TextRange.m4401getMaximpl(value.getSelection()));
            z3 = true;
            z = false;
        }
        if (value.getComposition() == null) {
            this.mBuffer.commitComposition$ui_text_release();
        } else if (!TextRange.m4398getCollapsedimpl(value.getComposition().getPackedValue())) {
            this.mBuffer.setComposition$ui_text_release(TextRange.m4402getMinimpl(value.getComposition().getPackedValue()), TextRange.m4401getMaximpl(value.getComposition().getPackedValue()));
        }
        if (z || (!z3 && z2)) {
            this.mBuffer.commitComposition$ui_text_release();
            value = TextFieldValue.m4618copy3r_uNRQ$default(value, (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null);
        }
    ...
    private final String generateBatchErrorMessage(List<? extends EditCommand> editCommands, final EditCommand failedCommand) {
        ...
        StringBuilder append = sb.append("Error while applying EditCommand batch to buffer (length=" + this.mBuffer.getLength$ui_text_release() + ", composition=" + this.mBuffer.m4545getCompositionMzsxiRA$ui_text_release() + ", selection=" + ((Object) TextRange.m4407toStringimpl(this.mBuffer.m4546getSelectiond9O1mEE$ui_text_release())) + "):");
    ...
    public final TextFieldValue apply(List<? extends EditCommand> editCommands) {
        ...
        try {
            ...
            TextFieldValue textFieldValue = new TextFieldValue(this.mBuffer.toAnnotatedString$ui_text_release(), this.mBuffer.m4546getSelectiond9O1mEE$ui_text_release(), this.mBuffer.m4545getCompositionMzsxiRA$ui_text_release(), (DefaultConstructorMarker) null);
