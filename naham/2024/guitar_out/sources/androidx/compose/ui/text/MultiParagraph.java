...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics intrinsics, long j, int i, boolean z) {
        boolean z2;
        int m4828getMaxHeightimpl;
        Intrinsics.checkNotNullParameter(intrinsics, "intrinsics");
        this.intrinsics = intrinsics;
        this.maxLines = i;
        if (Constraints.m4831getMinWidthimpl(j) != 0 || Constraints.m4830getMinHeightimpl(j) != 0) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m4824getHasBoundedHeightimpl(j)) {
                m4828getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m4828getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                m4828getMaxHeightimpl = Constraints.m4828getMaxHeightimpl(j);
            }
            Paragraph m4304Paragraph_EkL_Y = ParagraphKt.m4304Paragraph_EkL_Y(intrinsics2, ConstraintsKt.Constraints$default(0, m4829getMaxWidthimpl, 0, m4828getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + m4304Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + m4304Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(m4304Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (m4304Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m4829getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m4290paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m2665getUnspecified0d7_KjU();
        }
        multiParagraph.m4295paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m4295paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            paragraphInfo.getParagraph().mo4267paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m4294paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo4266paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m4296painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(brush, "brush");
        AndroidMultiParagraphDrawKt.m4628drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int findParagraphByIndex = MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, start); findParagraphByIndex < size; findParagraphByIndex++) {
            ...
            if (paragraphInfo.getStartIndex() != paragraphInfo.getEndIndex()) {
                Path.m2893addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        }
        return Path;
    }
    ...
    public final int m4292getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m2391getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m2391getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m2391getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return Math.max(0, paragraphInfo.getStartIndex() - 1);
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo4264getOffsetForPositionk4lQ0M(paragraphInfo.m4301toLocalMKHz9U(position)));
    }
    ...
    public final long m4293getWordBoundaryjx7JFs(int offset) {
        int findParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            findParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(findParagraphByIndex);
        return paragraphInfo.m4300toGlobalGEjPoXI(paragraphInfo.getParagraph().mo4265getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
