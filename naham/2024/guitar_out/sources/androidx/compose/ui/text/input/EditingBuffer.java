...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString text, long j) {
        ...
        this.selectionStart = TextRange.m4402getMinimpl(j);
        this.selectionEnd = TextRange.m4401getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m4402getMinimpl = TextRange.m4402getMinimpl(j);
        int m4401getMaximpl = TextRange.m4401getMaximpl(j);
        if (m4402getMinimpl < 0 || m4402getMinimpl > text.length()) {
            throw new IndexOutOfBoundsException("start (" + m4402getMinimpl + ") offset is outside of text region " + text.length());
        }
        if (m4401getMaximpl < 0 || m4401getMaximpl > text.length()) {
            throw new IndexOutOfBoundsException("end (" + m4401getMaximpl + ") offset is outside of text region " + text.length());
        }
        if (m4402getMinimpl > m4401getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + m4402getMinimpl + " > " + m4401getMaximpl);
        }
    }
    ...
    public final TextRange m4545getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m4392boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m4546getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long m4547updateRangeAfterDeletepWDy79M = EditingBufferKt.m4547updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m4402getMinimpl(m4547updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m4401getMaximpl(m4547updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m4547updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4547updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m4398getCollapsedimpl(m4547updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m4402getMinimpl(m4547updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m4401getMaximpl(m4547updateRangeAfterDeletepWDy79M2);
            }
        }
    }
