...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public final void m4659setSizeuvyYCjk(long j) {
        this.size = j;
    }
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float f) {
        ...
        this.alpha = f;
        this.size = Size.INSTANCE.m2467getUnspecifiedNHjbRc();
    }

    @Override // android.text.style.CharacterStyle
    public void updateDrawState(TextPaint textPaint) {
        Shader mo2598createShaderuvyYCjk;
        Intrinsics.checkNotNullParameter(textPaint, "textPaint");
        AndroidTextPaint_androidKt.setAlpha(textPaint, this.alpha);
        if (this.size == Size.INSTANCE.m2467getUnspecifiedNHjbRc()) {
            return;
        }
        Pair<Size, ? extends Shader> pair = this.cachedShader;
        if (pair == null || !Size.m2455equalsimpl0(pair.getFirst().getPackedValue(), this.size)) {
            mo2598createShaderuvyYCjk = this.shaderBrush.mo2598createShaderuvyYCjk(this.size);
        } else {
            mo2598createShaderuvyYCjk = pair.getSecond();
        }
        textPaint.setShader(mo2598createShaderuvyYCjk);
        this.cachedShader = TuplesKt.to(Size.m2447boximpl(this.size), mo2598createShaderuvyYCjk);
    }
}
