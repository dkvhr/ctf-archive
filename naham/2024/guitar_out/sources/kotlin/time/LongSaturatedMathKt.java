...
public final class LongSaturatedMathKt {
    ...
    public static final long m6615saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m6532toLongimpl = Duration.m6532toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m6614checkInfiniteSumDefinedPjuGub4(j, j2, m6532toLongimpl);
        }
        if ((1 | (m6532toLongimpl - 1)) == Long.MAX_VALUE) {
            return m6616saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m6532toLongimpl;
        return ((j ^ j3) & (m6532toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m6614checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m6518isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m6616saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m6489divUwyO8pc = Duration.m6489divUwyO8pc(j2, 2);
        long m6532toLongimpl = Duration.m6532toLongimpl(m6489divUwyO8pc, durationUnit);
        return (1 | (m6532toLongimpl - 1)) == Long.MAX_VALUE ? m6532toLongimpl : m6615saturatingAddNuflL3o(m6615saturatingAddNuflL3o(j, durationUnit, m6489divUwyO8pc), durationUnit, Duration.m6521minusLRDsOJo(j2, m6489divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m6588getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m6587getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m6539unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m6522plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m6539unaryMinusUwyO8pc(infinityOfSign(j3));
        }
