		...
		switch self.destination {
			case 0:
				...
				if sm.set_base + 5 > 31 {
					...
					let bottom_base = (sm.set_base + 5) & 0x1f;

					// mask and set the bottom part
					let bottom_mask: UInt32 = 0x1f >> (5 - bottom_base);
					pio.gpio_pins = (pio.gpio_pins & (~bottom_mask)) | (self.data >> (5 - bottom_base));
				} else {
					// simple - mask, set, update
			...
			case 4:
				...
				if sm.set_base + 5 > 31 {
					...
					// mask and set the bottom part
					let bottom_mask: UInt32 = 0x1f >> (5 - bottom_base);
					pio.gpio_pindirs = (pio.gpio_pindirs & (~bottom_mask)) | (self.data >> (5 - bottom_base));
				} else {
					// simple - mask, set, update
					let mask: UInt32 = 0x1f << sm.set_base;
...
class PIO {
	...
	// should this PIO share GPIO pins with the other
	var share_pins : Bool = false;
	...
	func execute() throws {
		...
		for _ in 0...self.nsteps {
			for pio in self.pios {
				...
				// sync GPIO pins
				if pio.share_pins {
					for other_pio in self.pios {
						if other_pio.share_pins {
							other_pio.gpio_pins = pio.gpio_pins;
						}
					}
				}
...
func receive_input(data: String, sm: StateMachine) throws {
	...
	for (k, v) in userinput! {
		switch k {
			...
			case "gp":
				sm.pio.gpio_pins = v as! UInt32
			case "gpd":
				sm.pio.gpio_pindirs = v as! UInt32
			case "isd":
...
func setup_game(tx: String, target: [UInt32]) throws -> [PIO] {
	...
	do {
		let sm = srcpio.state_machines[0];

		try receive_input(data: tx, sm: srcpio.state_machines[0])

		// wrap after we finish our loaded instructions
		sm.wrap_top = UInt32(srcpio.instruction_memory.count - 1);
	...
	do {
		...
		let sm = destpio.state_machines[0];

		try receive_input(data: readLine()!, sm: destpio.state_machines[0])

		// wrap after we finish our loaded instructions
		sm.wrap_top = UInt32(destpio.instruction_memory.count - 1);
		sm.wrap_bottom = UInt32(0);

		// // get an deep output fifo
		try sm.rx_fifo.set_depth(depth: 0x100);

		// connect our input fifo to the output fifo of the srcpio SM
		sm.tx_fifo = srcpio.state_machines[0].rx_fifo;
		try sm.rx_fifo.set_depth(depth: 0x100);
	...
	return [srcpio, destpio];
}

for level in 1..<4 {
	let tx = try! String(contentsOf: URL(fileURLWithPath: "level\(level).json"), encoding: String.Encoding.ascii);
	print("level \(level): \(tx)")
	// generate a series of random values we want the user to receive
	var target : [UInt32] = [];
	for _ in 0..<8 {
		target.append(UInt32(Int.random(in: 0..<0xfff)))
	}

	let pios = try setup_game(tx: tx, target: target);

	// ok, execute it
	let game = Game(name: "im-pio-ssible", pios: pios)

	game.nsteps = 0x800000
	...
	for v in 0..<(target.count) {
		switch pios[1].state_machines[0].rx_fifo.pull() {
			case .failure:
				print("bad");
				exit(1);
			case .success(let val):
...
print("res: \(try! String(contentsOf: URL(fileURLWithPath: "/flag"), encoding: String.Encoding.ascii))")
